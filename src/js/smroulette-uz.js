        ...
        const e = { l: "l", an: "an", discText: ["скидка 50%", "скидка 40%", "скидка 20%", "скидка 18%"], di: "di", ng: "ng", sectionText: { respondText: "Наш менеджер свяжется с вами, чтобы принять заказ и сообщит подробнее о выигрыше.", placeholderText: "Введите номер", phoneMask: "+7(000)000-00-00", agreement: { isRequired: !1, isLink: !1 } }, cookiesLife: 1, a: "a", r: "r", res: '<p>Скрипт не будет работать, если убрать ссылку на разработчика. Чтобы использовать виджет без этой ссылки, приобретите полную версию <a href="https://smartlanding.biz/smartroulette-lp/">тут!</a></p>' },
            a = {
                createTextArea() {
                    ...
                    h.blur(function() { t(this).val().length === s.length ? (f.text(""), v.input = !0, g()) : t(this).val().length >= 1 && (v.input = !1, g(), f.text("Телефон рақами тўғри еканлигига ишонч ҳосил қилинг")) }), h.keyup(function() { t(this).val().length !== s.length && 0 !== t(this).val().length || f.text("") }), m.append(h).append(f);
                    const k = t("<input>").attr({ type: "hidden", name: "section[]", value: e.discText }),
                        _ = t("<button>").attr({ class: "smart-roulette__button" }).html('<div class="smart-roulette__button-inner">Айлантириш</div>'),
                    ...
                    if (d.append(u).append(p).append(b).append(k), o.isRequired) {
                        ...
                        let a = t("<label>").attr({ class: "smart-roulette__label-agreement", for: "smart-roulette__input-agreement" }),
                            r = t("<input>").attr({ type: "checkbox", name: "agreement", class: "smart-roulette__input-agreement", id: "smart-roulette__input-agreement", checked: !0, value: "Фойдаланувчи шартномасини қабул қиламан" }),
                            n = t("<div>").attr({ class: "smart-roulette__input-msgs" }),
                            s = null;
                        s = o.isLink ? t("<a>").attr({ class: "smart-roulette__link-agreement", href: o.isLink, target: "_blank", rel: "nofollow" }) : t("<span>").attr({ class: "smart-roulette__link-agreement" }), r.is(":checked") ? s.text("Фойдаланувчи шартномасини қабул қиламан") : s.text("Фойдаланувчи шартномасини қабул қилмайман"), a.click(function() { r.is(":checked") ? (s.text("Фойдаланувчи шартномасини қабул қилмайман"), r.val("Фойдаланувчи шартномасини қабул қилмайман"), v.checkbox = !1, g(), n.text("Шартномани қабул қилинг")) : (s.text("Фойдаланувчи шартномасини қабул қиламан"), r.val("Фойдаланувчи шартномасини қабул қиламан"), v.checkbox = !0, g(), n.text("")) }), e.append(r).append(a).append(s).append(n), d.append(e)
                    }
                    if (a.spinDrum(_), 4 !== e.discText.length) l.text("Для работы виджета необходимо заполнить 4 ячейки в файле smart-roulette-config.js!");
                    else if (null == t.cookie("smr")) console.log("Нет куки"), a.v() ? l.append(d).append(c) : l.text(e.res);
                    else {
                        const e = t("<div>").attr({ class: "smart-roulette__text-respond" }).html(`\n                            <div class="smart-roulette__title-respond">Сиз ғилдиракни айлантириб бўлдингиз!</div>\n                            <div class="smart-roulette__prize-respond">Ваш выигрыш: ${t.cookie("smr")}</div>\n                            <div class="smart-roulette__subtitle-respond">${r}</div>\n                        `).css("display", "block");
                        l.append(e), console.log("Есть кука")
                    }
                ...
                createModal() {
                    ...
                    return a.v() ? e.append(a.v()) : alert("Нельзя уждалять ссылку на сайт разработчика"), r.append(n), e.append(r.append(a.createRoulette()).append(a.createTextArea)), t(".smart-roulette__gift, .smart-roulette__gift-button").click(function() { t(".smart-roulette").addClass("smart-roulette_active"), t("body").css("overflowY", "hidden") }), n.click(function() { t(".smart-roulette").removeClass("smart-roulette_active"), t("body").css("overflowY", "auto") }), e.mouseup(function(a) { r.is(a.target) || 0 !== r.has(a.target).length || (e.removeClass("smart-roulette_active"), t("body").css("overflowY", "scroll")) }), e
                },
                ...
                spinDrum: function(a) {
                    a.click(function(r) {
                        ...
                        t.ajax({
                            ...
                            success: function(a) {
                                let r = t.parseJSON(a);
                                if (console.log(r), !isNaN(parseFloat(r.deg)) && isFinite(r.deg)) console.log("число" + r.deg), p.css({ transform: "rotate(" + r.deg + "deg)" }), Number.isInteger(e.cookiesLife) && t.cookie("smr", r.title, { expires: e.cookiesLife }), setTimeout(() => { d.css("display", "none"), f.text(r.title), m.css("display", "block"), e.met(), console.log(r.title) }, 6100);
                                else switch (r.error) {
                                    case "NOTCONTACT":
                                        console.log(r.error), v.text("Введите корректный контакт для связи").css({ opacity: 1, visibility: "visible" });
                                        break;
                                    case "NOTSECTION":
                                        console.log("Не передались секции, проверьте настройки скрипта");
                                        break;
                                    case "NOTCORRECTMAIL":
                                        console.log(r.error), v.text("Введите корректный email").css({ opacity: 1, visibility: "visible" })
                                }
                            },
                            complete: function(t) {}
                        })
                ...
                init: function(r) { t.extend(e, r); const n = t("<button>").attr({ class: "smart-roulette__gift smart-roulette__gift_left" }); return n.append('<img src="img/gift.svg"  width="80" alt="Подарок" title="Совғангизни олинг!">'), this.each(function() { t(this).append(n), t(this).append(a.createWidget()) }) }
            };
        t.fn.smroulette = function(e) { return a[e] ? a[e].apply(this, Array.prototype.slice.call(arguments, 1)) : "object" != typeof e && e ? void t.error("Метод " + e + " не найден") : a.init.apply(this, arguments) }
